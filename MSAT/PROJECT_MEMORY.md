...
- 2026-06-28/29 已完善 `MSAT/scripts/run_contribution_quantification.py`、`MSAT/results/PU_XMSAT_CONTRIBUTION_QUANTIFICATION.md`、`MSAT/results/contribution_quantification.json` 和 `.csv`，完成解释层关键机制子图与贡献量化。方法是从机制路径抽取 compound/target 节点，形成机制子图，并分别进行节点置零与路径置零重评分；`--max-features 0` 现在表示不截断、量化完整解析节点。当前量化 2 个案例：枳实-水样腹泻子图含 11 个节点、8 条边和 14 条路径，节点覆盖 `11/11`；最高节点为 `target:3223`（drop 0.009835），最高路径为 `compound:523 -> target:3223`（drop 0.010074）。野草莓-意识状态改变子图含 2 个节点、1 条边和 1 条路径，节点覆盖 `2/2`，最大下降仅 0.000021。负的 score drop 表示遮蔽后分数反而上升，不能解释为保护性生物学。该结果只能写作本地 `saved_models/best_model_for_prediction.pt` predictor checkpoint 下的局部敏感性分析，不是 SHAP、不是因果效应，也不是最终 full-positive hybrid PU checkpoint 的归因。若未来要写“PU-XMSAT 归因”，需要先导出明确的 PU predictor checkpoint 并重新运行该脚本。
- 2026-06-29 已新增并刷新 `MSAT/scripts/summarize_contribution_quantification.py`、`MSAT/results/PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY.md`、`MSAT/results/contribution_aggregate_summary.json` 和 `.csv`，把方向二的节点/路径扰动结果聚合成论文可用 top path/top node 表。当前聚合覆盖 2 个机制案例，共 11 条正向 node 扰动和 10 条正向 path 扰动；最强路径为 `compound:523;target:3223`（mean/max drop 0.010074），最强节点为 `target:3223`（drop 0.009835）。未发现显式 final hybrid PU predictor checkpoint，因此该聚合仍只能写成本地 predictor checkpoint 下的 perturbation sensitivity aggregate，不是因果、不是 SHAP、也不是最终 PU-XMSAT checkpoint attribution。
- 2026-06-29 已新增 `MSAT/scripts/build_mechanism_explanation_layer.py`、`MSAT/results/PU_XMSAT_MECHANISM_EXPLANATION_LAYER.md`、`MSAT/results/mechanism_explanation_layer.json` 和 `.csv`，作为导师/论文可读的解释层总入口。它确认当前 2 个机制案例均已提取关键子图且完整节点均已量化，并把贡献拆成三类：4 个成分贡献特征（3 个正向，最强 `compound:523`，mean drop 0.000239）、9 个靶点贡献特征（8 个正向，最强 `target:3223`，mean drop 0.009835）、10 个机制路径特征（8 个正向，最强 `compound:523;target:3223`，mean/max drop 0.010074）。这是当前“解释层面：关键机制子图 + 成分/靶点/路径贡献”的权威入口。
- 2026-06-29 已新增 `MSAT/scripts/build_direction3_targeted_review_queue.py`、`MSAT/results/PU_XMSAT_DIRECTION3_TARGETED_REVIEW_QUEUE.md`、`MSAT/results/direction3_targeted_review_queue.json` 和 `.csv`，把方向二扰动敏感性转成方向三定向证据复核队列。当前队列包含 2 个已量化案例：`Citrus aurantium L. -> Watery diarrhoea` 以 path drop `0.0100741684` 排第 1，`Fragaria vesca L. -> Altered state of consciousness` 以 path drop `0.0000212789` 排第 2；两者都已有人工核验、仍为 Grade C boundary cases，`ready_strong_evidence_count=0`。`Template/PU-XMSAT-Overleaf/main.tex` 的 Evidence Screening 段已加入这条保守队列结论；这强化的是“筛选和优先级排序”证据链，不是外部验证成功。
- 2026-06-29 已升级解释性产物为 checkpoint-aware batch 机制解释：新增 `MSAT/scripts/run_batch_mechanism_interpretability.py`、`MSAT/tests/test_run_batch_mechanism_interpretability.py`、`MSAT/results/PU_XMSAT_INTERPRETABILITY_GAP_AUDIT.md`、`MSAT/results/PU_XMSAT_BATCH_MECHANISM_INTERPRETABILITY.md`、`MSAT/results/batch_mechanism_interpretability.json` 和 `.csv`。当前 batch 默认 top-K 为 20，但只有 2 个可解析 explicit mechanism path candidates 可用；候选来源是 transitional mechanism-supported artifacts，不是 final PU-XMSAT top-ranking export。由于没有 final full-positive hybrid PU-XMSAT predictor checkpoint，默认复用本地 `saved_models/best_model_for_prediction.pt` 下已有贡献行，实际 perturbation-quantified cases 为 2，unquantified explicit-path candidates 为 0。最强 component 为 `compound:523`（drop `0.0002386272`），最强 target 为 `target:3223`（drop `0.0098346472`），最强 pathway 为 `compound:523;target:3223`（drop `0.0100741684`）。当前 Grade A/B 强外部证据仍为 0。`run_pu_msat_experiment.py` 现在暴露 `--save-checkpoints`、`--checkpoint-dir`、`--checkpoint-prefix`；默认 formal checkpoint prefix 包含 backend、strategy、seed、pair budget、threshold strategy、unlabeled weight 和 reliable-negative weight，文件名追加 fold，避免覆盖 MSAT baseline checkpoint。当前解释结论仍不能写成 SHAP、因果、临床验证或最终 PU checkpoint attribution。
...
2. 不建议继续长训，除非用户明确要求。当前交付物总入口为 `MSAT/results/PU_XMSAT_DELIVERABLE_INDEX_CN.md`；解释 checkpoint gap 审计和 batch 解释已整理到 `MSAT/results/PU_XMSAT_INTERPRETABILITY_GAP_AUDIT.md`、`MSAT/results/PU_XMSAT_BATCH_MECHANISM_INTERPRETABILITY.md`；论文结果表和 claim 边界已整理到 `MSAT/results/PU_XMSAT_MANUSCRIPT_RESULTS_DRAFT.md`，Methods/Results/Discussion 正文草稿已整理到 `MSAT/results/PU_XMSAT_MANUSCRIPT_SECTIONS_DRAFT.md`，Overleaf 论文包审计已整理到 `MSAT/results/PU_XMSAT_MANUSCRIPT_PACKAGE_AUDIT.md`，导师沟通版中文汇报已整理到 `MSAT/results/PU_XMSAT_MENTOR_PROGRESS_BRIEF_CN.md`，3-5 分钟口头稿已整理到 `MSAT/results/PU_XMSAT_ORAL_BRIEF_CN.md`，组会/答辩 slides 大纲已整理到 `MSAT/results/PU_XMSAT_SLIDES_OUTLINE_CN.md`，PPT 初稿已生成到 `MSAT/results/PU_XMSAT_SLIDES_DRAFT_CN.pptx`，解释/证据闭环已整理到 `MSAT/results/PU_XMSAT_CASE_EVIDENCE_REPORT.md`，Grade C 人工核验已整理到 `MSAT/results/PU_XMSAT_GRADE_C_MANUAL_EVIDENCE_AUDIT.md`，案例选择决策已整理到 `MSAT/results/PU_XMSAT_CASE_SELECTION_DECISION.md`，关键机制子图与节点/路径扰动贡献量化已整理到 `MSAT/results/PU_XMSAT_CONTRIBUTION_QUANTIFICATION.md`，batch 聚合摘要已整理到 `MSAT/results/PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY.md`，解释层总入口已整理到 `MSAT/results/PU_XMSAT_MECHANISM_EXPLANATION_LAYER.md`，因果偏倚边界已整理到 `MSAT/results/PU_XMSAT_CAUSAL_BIAS_FRAMEWORK.md`，研究闭环审计已整理到 `MSAT/results/PU_XMSAT_RESEARCH_CLOSURE_AUDIT.md`；下一步若要增强案例证据，应先导出 final hybrid PU checkpoint 和 final PU top-ranking candidates，再寻找更合适的高置信候选，而不是继续盲目长训或扩大无目的检索。
