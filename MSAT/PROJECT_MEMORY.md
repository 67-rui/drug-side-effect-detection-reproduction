...
- 2026-06-29 已新增 `MSAT/scripts/summarize_contribution_quantification.py`、`MSAT/results/PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY.md`、`MSAT/results/contribution_aggregate_summary.json` 和 `.csv`，把方向二的节点/路径扰动结果聚合成论文可用 top path/top node 表。当前聚合覆盖 2 个机制案例，共 7 条正向 node 扰动和 10 条正向 path 扰动；最强路径为 `compound:523;target:3223`（mean/max drop 0.010074），最强节点为 `target:3223`（drop 0.009835）。未发现显式 final hybrid PU predictor checkpoint，因此该聚合仍只能写成本地 predictor checkpoint 下的 perturbation sensitivity aggregate，不是因果、不是 SHAP、也不是最终 PU-XMSAT checkpoint attribution。
