...
1. 为 `full_msat_pu` 增加更大训练 pair budget 的可控配置，优先支持 3,072 / 6,144 / 12,288 pairs，或显式支持 full-positive budget。
2. 先用 `hybrid` 跑 fold0 budget scaling，因为它是当前 bounded 10-fold ranking 指标最好策略。
