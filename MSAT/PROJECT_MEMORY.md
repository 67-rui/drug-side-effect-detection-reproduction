...
10. 2026-06-29 已新增论文 case-study triage 表 `MSAT/results/PU_XMSAT_CASE_STUDY_TRIAGE_TABLE.md`，整理三例：`Polypodium glycyrrhiza` -> `Watery diarrhoea` 作为正向机制 triage，`Marchantia polymorpha` -> `Liver injury` 作为 PubMed 文本命中但方向冲突的证据复核案例，`Fragaria vesca L.` -> `Altered state of consciousness` 作为边界/负向案例。Marchantia liver injury 有 final top5000 预测和 8 条显式路径，但本地只有 formal checkpoint metadata、没有对应 final `.pt` 权重；因此不能写成已完成 formal final targeted perturbation，除非后续恢复该 checkpoint 并单独重跑。
