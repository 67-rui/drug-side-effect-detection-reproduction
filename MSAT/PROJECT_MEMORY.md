...
- 2026-06-29 已将英文 ACM 风格论文工程持久化到 `Template/PU-XMSAT-Overleaf/`，并保留上传包 `Template/PU-XMSAT-Overleaf.zip`。新增 `MSAT/results/PU_XMSAT_MANUSCRIPT_PACKAGE_AUDIT.md` 记录论文包位置、模板合规检查、占位项和打包规则。当前稿件编译为 12 页 Letter PDF；源码使用提供的 `acmart.cls`，zip 不包含 `main.pdf` 或 LaTeX 辅助产物。`.gitignore` 只忽略原始模板包 `Template/文件*`、本地预览 `Template/PU-XMSAT-Overleaf/main.pdf`、`.DS_Store` 和无关共享 zip，避免把用户提供的原始模板附件或本地编译产物混进提交。
