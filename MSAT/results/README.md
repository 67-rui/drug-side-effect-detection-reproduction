...
| `PU_XMSAT_FULL_MSAT_PILOT_REPORT.md` | PU-XMSAT full-backend pilot | full MSAT PU 后端可运行；旧 bounded pilot 使用 prefix candidate cache，适合作为训练闭环诊断，不作为最终策略优劣结论 |
| `PU_XMSAT_RESEARCH_PROGRESS_REPORT.md` | 后续论文素材积累 | 记录 PU-XMSAT 研究动机、fold0/3-fold/10-fold pilot、candidate cache 审计、下一步 randomized-cache budget scaling |
...
3. PU-XMSAT 下一步不要继续使用旧 prefix candidate cache；应使用随机化后的 50k candidate cache，优先用 `hybrid` 做 fold0 budget scaling，再决定是否进入新一轮多折。
