...
| `PU_XMSAT_FULL_MSAT_PILOT_REPORT.md` | PU-XMSAT full-backend pilot | full MSAT PU 后端可运行；corrected random50k full-positive random/10-fold seed=42 达到 AUC 0.9796、AUPRC 0.9773、F1 0.9321、MCC 0.8625；seed=2026 稳健性复跑达到 AUC 0.9797、AUPRC 0.9777、F1 0.9338、MCC 0.8661；当前为 robust baseline-level，但提升仍小且未达统计显著 |
| `PU_XMSAT_RESEARCH_PROGRESS_REPORT.md` | 后续论文素材积累 | 记录 PU-XMSAT 研究动机、prefix-cache caveat、corrected random50k budget scaling、10-fold corrected hybrid/random/full-positive pilot、repeated-seed robustness 和下一步稳健性计划 |
...
3. PU-XMSAT 下一步不要继续使用旧 prefix candidate cache，也不要重复已完成的 corrected 12,288-pair、66,015-pair `random` 10-fold 对照或 seed=2026 repeated-seed run；paired fold/statistical note 和 repeated-seed note 已生成，后续可选择 PU weight sensitivity 或 full-positive `hybrid` comparator。
