...
| `reproduction_gap_diagnosis.json` | Fig.5a/Table5 差异诊断 | 可引用；Table5 论文配对 15 行中 14 行可映射，只有 1 行进入 OOF 分数；本地 checkpoint 与服务器 Table5 manifest 不一致 |
...
1. Table 5 下一步应先同步服务器 Table5 checkpoint，随后反推论文 §3.5.6 的候选生成方式；当前论文 15 个配对只有 1 个进入 OOF 分数，公开文献自动候选也未发现精确支持。
