...
| `PU_XMSAT_INTERPRETABILITY_GAP_AUDIT.md` | 解释 checkpoint gap 审计 | 当前解释 checkpoint 是 `saved_models/best_model_for_prediction.pt`，不是 final full-positive hybrid PU-XMSAT checkpoint；`run_pu_msat_experiment.py` 已支持安全 checkpoint 导出和正式命名 |
| `PU_XMSAT_BATCH_MECHANISM_INTERPRETABILITY.md` / `batch_mechanism_interpretability.json` / `.csv` | Batch 机制解释 | 默认 top-K 为 20；当前从 transitional mechanism artifacts 选择 2 个可解析显式机制路径候选，因没有 final PU checkpoint，复用已有贡献行实际扰动量化 2 个案例；top component 为 `compound:523`，top target 为 `target:3223`，top pathway 为 `compound:523;target:3223`；不是 final PU-XMSAT top-ranking export |
| `PU_XMSAT_CONTRIBUTION_QUANTIFICATION.md` / `contribution_quantification.json` / `.csv` | 关键机制子图与贡献量化 | 已完成 2 个机制案例的关键子图、完整节点置零和路径置零扰动评分；枳实案例子图含 11 个节点/8 条边/14 条路径且节点覆盖 11/11，最高节点为 `target:3223`（drop 0.009835），最高路径为 `compound:523 -> target:3223`（drop 0.010074）；野草莓案例节点覆盖 2/2，扰动下降接近 0；这是本地 predictor checkpoint 下的敏感性分析，不是因果效应，也不是最终 PU checkpoint 归因 |
| `PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY.md` / `contribution_aggregate_summary.json` / `.csv` | 贡献量化聚合摘要 | 已升级为可读取 batch interpretability JSON：当前 2 个可解析显式路径候选均有扰动量化，聚合 top component/target/pathway；最强成分 `compound:523`、靶点 `target:3223`、路径 `compound:523;target:3223`；区分 positive、near-zero、negative score drop；仍是本地 predictor checkpoint 下的 perturbation sensitivity |
| `PU_XMSAT_MECHANISM_EXPLANATION_LAYER.md` / `mechanism_explanation_layer.json` / `.csv` | 解释层总入口 | 把关键机制子图、成分贡献、靶点贡献和机制路径贡献拆分成导师/论文可读表；当前 2 个机制案例均完整量化，贡献拆分为 4 个成分特征（3 个正向）、9 个靶点特征（8 个正向）和 10 个机制路径特征（8 个正向） |
| `PU_XMSAT_DIRECTION3_TARGETED_REVIEW_QUEUE.md` / `direction3_targeted_review_queue.json` / `.csv` | 方向三定向证据复核队列 | 默认读取 batch interpretability：当前 2 个候选入队，按 max positive path drop、target drop、component drop 排序；枳实-水样腹泻排第 1，野草莓-意识状态改变排第 2，不存在无扰动行候选；Grade A/B 为 0，ready strong-evidence cases 为 0 |
