...
| `PU_XMSAT_NEXT_PUBLICATION_ACTION_REPORT.md` | 投稿前推进报告 | 汇总当前已完成内容、论文可写内容、限制/future work、解释性完成度、case-study 候选、cluster-heldout 论文作用、投稿前 todo、等待原作者事项和提交/推送建议 |
...
| `PU_XMSAT_CLUSTER_HOLDOUT_GENERALIZATION.md` / `cluster_holdout_generalization_summary.json` / `pu_xmsat_cluster_holdout_hybrid_seed2026_10cluster_200e_66015p_valf1.json` | Package B cluster-held-out 正式泛化实验 | 已完成 full-positive hybrid seed=2026 的 10-cluster/10-fold/200-epoch 留簇测试；all-fold mean AUC/AUPRC/F1/MCC 为 0.8891/0.9032/0.1770/0.1919；排除少于 5 味中药 heldout clusters 后的 non-tiny robust mean 为 0.8433/0.8633/0.0743/0.1168。heldout herbs 已从 PU 训练数组过滤，eval positive CMM-ADR edges 已隐藏。该实验应写成更严格的新中药簇泛化压力测试：排序指标仍有信号，但阈值型指标明显下降，且 3 个 heldout clusters 少于 5 味中药，macro mean 需结合 fold-level cluster size 与 non-tiny summary 解读 |
...
| `PU_XMSAT_TOP20_ENTITY_MAPPING_QUEUE.md` / `top20_entity_mapping_queue.json` / `.csv` | top-20 解释候选实体映射队列 | 从 top5000/random-control batch 解释中抽取 20 个 compound 和 39 个 target；当前 confirmed mapped_count=0、unmapped_count=59，明确 compound/target 仍为 MSAT graph-local IDs |
| `PU_XMSAT_TOP20_TARGET_NAME_CANDIDATES.md` / `top20_target_name_candidates.json` / `.csv` | top-20 target 候选名称对齐 | 使用 HGNC complete set 与 target BioBERT 节点向量做候选对齐；39 个 target 均生成 `candidate_only` 候选，不能当作原始图 ID 映射或外部验证 |
| `PU_XMSAT_ENTITY_MAPPING_AUDIT_AND_CANDIDATES.md` | 解释实体映射审计 | 总结为什么当前不能直接把 `compound:<id>` / `target:<id>` 还原为真实名称；ccTCM/ETCM/HGNC/ADReCS 等只能作为候选或外部证据源，不能自动证明本地 ID |
...
3. PU-XMSAT 下一步不要继续使用旧 prefix candidate cache，也不要重复已完成的 corrected 12,288-pair、66,015-pair `random` 10-fold 对照、seed=2026 repeated-seed run、full-positive `hybrid` seed=2026/1337 comparator 或 u0.1/u0.4 weight sensitivity。Package A/B/C 的正式证据已收束：主结果整合、10-cluster cluster-heldout 泛化压力测试、top5000/random-control batch 解释、top20 实体映射审计、target 候选名称、top20 外部证据 first-pass 复核和 391 候选 evidence-aware 重排序均已有可审计产物。若要继续增强论文，应优先人工复核 evidence-aware top30 或写入 Overleaf 并保守表述解释边界，而不是回退到 local checkpoint 或 top-50 旧口径。
