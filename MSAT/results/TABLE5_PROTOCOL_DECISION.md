...
| Check | Current finding | Implication |
| --- | --- | --- |
| Paper §3.5.6 text | Table 5 is described as the Top-15 highest-confidence CMM-ADR predictions absent from labeled positives; lower-confidence strata ranks 21-100 and 101-200 are supplementary validation sets | The paper does not expose enough implementation detail to reconstruct the exact candidate generator from text alone |
| Released source code | `resource_副本/MSAT-main/` contains `train.py`, `model.py`, `config.py`, and the official fold split, but no Table 5 export/prediction script | Table 5 was likely generated by an unreleased downstream script or notebook |
