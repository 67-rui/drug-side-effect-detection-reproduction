...
For case-level interpretation, we added a perturbation-based contribution quantification step. The final interpretation run used the exported full-positive hybrid PU-XMSAT checkpoint and the complete top-5000 unobserved CMM-ADR prediction export. For each selected candidate with parseable explicit mechanism paths, compound and target node identifiers were extracted from the available paths and assembled into a key mechanism subgraph. We then quantified two levels of local sensitivity. First, the input feature vector of one candidate compound or target node was zeroed while keeping the graph structure and the queried CMM-ADR pair fixed. Second, all parsed compound and target nodes in a single mechanism path were zeroed together. The same final PU-XMSAT checkpoint was used to re-score the pair after each perturbation. The contribution score was defined as the original score minus the masked score. Random component, target, and pathway perturbation controls were generated to provide a baseline for interpreting the observed score drops.

This score should be interpreted only as local perturbation sensitivity. It is not a SHAP value, a causal effect, or direct evidence that the biological entity causes the adverse reaction. Negative score drops indicate that the score increased after masking and should be treated as suppressive or non-supportive sensitivity signals. Although the current run is final-checkpoint-aware and batch-level, explicit mechanism-path coverage remains sparse, so the analysis supports mechanism triage rather than broad mechanism confirmation.

### Cluster-Held-Out Generalization

To test whether the model generalizes beyond the official fold protocol, we added a cluster-held-out evaluation. Herbs/CMMs were clustered by their herb feature matrix, and one cluster was held out in each fold. Heldout herbs were removed from PU training positives, reliable negatives, and unlabeled arrays through an `allowed_train_herbs` filter. Validation and test positive CMM-ADR edges were hidden during evaluation, while mechanism edges were retained because the protocol tests generalization to held-out herb/CMM clusters rather than blind removal of all biological annotations. This experiment should be interpreted as a new-cluster generalization stress test.
...
### Cluster-Held-Out Evaluation Reveals a Harder Generalization Boundary

The cluster-held-out experiment evaluated full-positive hybrid PU-XMSAT under 10 heldout herb/CMM clusters. Mean AUC and AUPRC remained above random at 0.889069 and 0.903219, indicating that ranking signal survived the stricter split. However, thresholded metrics degraded sharply: mean F1 was 0.176976 and mean MCC was 0.191914. Fold-level cluster sizes were also imbalanced, with heldout herb counts ranging from 1 to 207 and three clusters containing fewer than five herbs. These results should be reported as evidence that new-cluster generalization is substantially harder than the official fold protocol, not as equivalent deployment performance on unseen herb groups.
...
The final-checkpoint perturbation contribution report quantified 20 mechanism-supported top-prediction cases and was further summarized into aggregate component, target, and pathway tables. In the complete top-5000 final PU-XMSAT prediction export, 391 candidates had parseable explicit mechanism paths, corresponding to 7.82% coverage. The selected 20 cases all had explicit mechanism paths and were fully quantified. Across the aggregate summary, the largest component-level score drop was observed for `compound:761` (mean drop 0.000309), the largest target-level drop for `target:15721` (drop 0.001174), and the largest pathway drop for `target:15721` (drop 0.001174). These identifiers currently use unmapped graph-id fallback display names, so they should be treated as graph features for follow-up rather than already validated biological names. The Direction 3 queue retained `ready_strong_evidence_count=0`. These results are useful for final-checkpoint-aware mechanism triage with random-control context, but they do not provide broad mechanism confirmation or confirmed adverse-reaction evidence.
...
The current case-level evidence report also remains intentionally conservative. Automated literature records are useful for screening, but they are not direct evidence until a human reviewer confirms the specific herb-ADR or mechanism claim. The final-checkpoint Direction 3 queue contains one Grade C boundary case and no ready strong-evidence case. It can support discussion of mechanism extraction and evidence triage, whereas Grade D rows and non-explicit-path top predictions should be retained only as candidate predictions for future review.

The contribution quantification analysis has the same boundary. It identifies which graph nodes and paths affect a local model score when perturbed, but the resulting score drop is not a causal estimate and does not validate the direction of an adverse reaction. The current report is final-checkpoint-aware and batch-level, but explicit-path coverage is sparse and 12 of 20 selected cases still show near-zero case-level sensitivity. It is best used to support a transparent interpretation workflow and to select candidates for stronger future evidence review.

The cluster-held-out result adds a separate generalization boundary. It is useful because it asks whether the PU-XMSAT signal survives when entire herb/CMM clusters are unseen during PU training. The observed AUC/AUPRC suggest remaining ranking signal, but the low F1/MCC and imbalanced cluster sizes mean it should be framed as a stress test and not as evidence of ready deployment on unseen herb groups.

### Next Manuscript Step

The next writing step should be to integrate these sections with the existing MSAT reproduction narrative. The paper should first establish that the reproduced MSAT baseline is protocol-aligned, then motivate incomplete-label noise, then introduce PU-XMSAT as a targeted extension. The Results section should report the full-positive hybrid two-seed table, the paired comparisons, the weight-sensitivity table, the cluster-heldout stress test, the conservative case evidence screening table, and the top5000 mechanism subgraph/node/path contribution table. The Discussion should emphasize that PU-XMSAT strengthens the reproduced baseline under incomplete-label assumptions while preserving conservative generalization, interpretation, and evidence boundaries.
...
- Package A consolidation: `results/PU_XMSAT_PUBLICATION_EVIDENCE_CONSOLIDATION.md`, `results/pu_xmsat_publication_evidence_consolidation.json`
- Cluster-heldout generalization: `results/PU_XMSAT_CLUSTER_HOLDOUT_GENERALIZATION.md`, `results/cluster_holdout_generalization_summary.json`
...
- Contribution quantification: `results/PU_XMSAT_CONTRIBUTION_QUANTIFICATION_TOP5000_RANDOM_CONTROLS.md`, `results/contribution_quantification_top5000_random_controls.csv`, `results/PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY_TOP5000_RANDOM_CONTROLS.md`, `results/contribution_aggregate_summary_top5000_random_controls.csv`, `results/PU_XMSAT_MECHANISM_EXPLANATION_LAYER_TOP5000_RANDOM_CONTROLS.md`, `results/mechanism_explanation_layer_top5000_random_controls.csv`
