...
The perturbation contribution report quantified two mechanism-supported cases and was further summarized into aggregate node/path tables plus a component-target-pathway explanation-layer handoff. In the Zhishi-diarrhoea case, the extracted key subgraph contained 11 nodes, 8 edges, and 14 mechanism paths, and all 11 parsed subgraph nodes were quantified. Across the aggregate summary, the largest node-level score drop was observed for target `3223` (drop 0.009835), and the largest path-level drop was observed for the `compound:523;target:3223` path (mean/max drop 0.010074). The current aggregate contains 11 positive node perturbation rows and 10 positive path perturbation rows. The split explanation summary contains 4 component features (3 positive), 9 target features (8 positive), and 10 mechanism-path features (8 positive), with `compound:523`, `target:3223`, and `compound:523;target:3223` as the strongest entries in their respective groups. Most positive signals are still concentrated in the Zhishi case rather than repeated across a larger case cohort. Several paths involving compound `875` had negative drops, indicating that masking those paths increased the local score rather than supporting the prediction. In the Fragaria-altered-consciousness case, the key subgraph contained only one parsed path with full node coverage 2/2, and the largest drop was 0.000021 for compound `1073`. These results are useful for prioritizing mechanism nodes and paths for review, but they do not change the conservative external-evidence conclusion: neither case should currently be presented as confirmed adverse-reaction evidence.
...
- Contribution quantification: `results/PU_XMSAT_CONTRIBUTION_QUANTIFICATION.md`, `results/contribution_quantification.csv`, `results/PU_XMSAT_CONTRIBUTION_AGGREGATE_SUMMARY.md`, `results/contribution_aggregate_summary.csv`, `results/PU_XMSAT_MECHANISM_EXPLANATION_LAYER.md`, `results/mechanism_explanation_layer.csv`
