...
RQ4 & Can the model output be connected to mechanism-level entities and paths? & Top-5000 mechanism coverage, key subgraph extraction, local node/path perturbation sensitivity, random controls, and aggregate sensitivity ranking for 20 mechanism-supported cases. & Supports mechanism prioritization; perturbation drops are not SHAP values, causal effects, or confirmed biological mechanisms. \\
RQ5 & Can case predictions be promoted to externally validated adverse-reaction evidence? & Entity-mapping audit, first-pass external evidence review for 20 mechanism-supported candidates, and evidence-aware top-30 reranking from the 391 explicit-path pool. & Current cases remain screening candidates; no complete herb--compound--target--ADR chain is externally validated. \\
\bottomrule
\end{tabularx}
\end{table*}
...
We presented \px{}, a positive-unlabeled extension of the reproduced \msat{} protocol for CMM adverse reaction prediction. The method improves the supervision assumption by separating observed positives, reliable negatives, and down-weighted unlabeled pairs. Under the reproduced ten-fold protocol, full-positive hybrid \px{} showed seed-robust improvements over the reproduced \msat{} baseline on AUC, F1, and MCC, with a stable positive AUPRC trend. A cluster-held-out stress test revealed a harder new-cluster generalization boundary, and a top-5000 mechanism-oriented perturbation analysis supports transparent case triage. The manuscript therefore positions \px{} as a conservative incomplete-label mitigation and mechanism-prioritization framework while maintaining boundaries around external validation and causal interpretation.
