...
RQ5 & Can case predictions be promoted to externally validated adverse-reaction evidence? & Evidence grading and manual review of available literature/database support for candidate cases. & Current cases remain screening candidates; no row is upgraded to confirmed external validation. \\
\bottomrule
\end{tabularx}
\end{table*}
...
\midrule
10-cluster heldout & 0.889069 & 0.903219 & 0.176976 & 0.191914 & Ranking signal remains, but thresholded generalization is weak \\
\bottomrule
\end{tabularx}
\end{table}
...
The case evidence report contained 16 rows: 15 existing \msat{} Table 5-style candidates and one curated Zhishi--diarrhoea case. Two rows had mechanism/path support and were assigned Grade C, while 14 rows remained prediction-only Grade D candidates. Eight rows had automated literature search records, but no row had manually verified direct literature support. After final-checkpoint top-5000 interpretation, the targeted Direction 3 review queue contained 20 mechanism-supported candidates: one boundary case with prior manual review and 19 cases requiring target-focused external evidence review. The ready-for-strong-evidence count remained 0. Therefore, the current evidence workflow should be described as screening and prioritization rather than external validation equivalent to the original Table 5 or Table 6 analyses.
...
Future work should expand the final-checkpoint contribution analysis beyond sparse explicit-path coverage by improving compound/target name mapping, using explicit-path-aware reranking, and conducting targeted external review of the 19 currently unreviewed mechanism-supported candidates. Generalization should be probed with more balanced cluster-held-out designs or repeated clustering seeds. If patient-level or pharmacovigilance report-level data become available, causal adjustment strategies could be explored to address co-medication, indication bias, reporting bias, and exposure population differences.
