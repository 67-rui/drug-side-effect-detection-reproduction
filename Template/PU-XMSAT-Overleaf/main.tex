...
For mechanism interpretation, we selected mechanism-supported candidates with an explicit priority order: final \px{} top-ranking predictions when available, otherwise transitional mechanism artifacts from the case evidence report, Table 5-style candidates, explanation case studies, and structured mechanism summaries. In the current artifact set, no final \px{} top-ranking export was available, so the batch analysis used mechanism-supported transitional candidates. Compound and target identifiers appearing in the available paths were assembled into a local subgraph for each selected CMM--ADR case. We then quantified two levels of local sensitivity where perturbation rows were available. First, the input feature vector of one candidate compound or target node was zeroed while keeping the graph structure and the queried CMM--ADR pair fixed. Second, all parsed compound and target nodes in a mechanism path were zeroed together. The contribution score was defined as the original prediction score minus the masked prediction score. We aggregated component, target, and pathway perturbation rows across the batch to rank sensitive mechanism entities and paths.

This procedure should be interpreted as local perturbation sensitivity. A positive drop indicates that masking the entity or path decreased the local model score. A negative drop indicates that the score increased after masking and should be treated as a suppressive or non-supportive sensitivity signal. The current batch report uses the local \texttt{saved\_models/best\_model\_for\_prediction.pt} predictor checkpoint rather than an explicitly exported final full-positive hybrid \px{} checkpoint, so the interpretation is restricted to a mechanism-prioritization workflow. The implementation now supports safe full-backend checkpoint export with formal names that include backend, sampling strategy, seed, fold, pair budget, threshold strategy, unlabeled weight, and reliable-negative weight, but such a final checkpoint was not used for the present attribution.
...
We generated a conservative batch mechanism-interpretation report (Table~\ref{tab:contribution}). Because no final \px{} top-ranking prediction export was present, the batch used mechanism-supported transitional candidates rather than final \px{} top predictions. Two candidates had parseable explicit mechanism paths, fewer than the requested top-20 batch size. Under the fallback local checkpoint context, both cases had reusable perturbation rows. In the Zhishi--diarrhoea case, the extracted key subgraph contained 11 nodes, 8 edges, and 14 mechanism paths, and all 11 parsed subgraph nodes were quantified. The largest target-level score drop was observed for target 3223 (drop 0.009835), and the largest pathway drop was observed for the compound 523 to target 3223 path (drop 0.010074). In the Fragaria--altered-consciousness case, the key subgraph contained two nodes and one edge, and both parsed nodes were quantified; the largest observed drop was 0.000021 and was classified as near-zero sensitivity. Aggregating the quantified rows yielded 3 positive node perturbation rows, 9 near-zero node rows, 1 negative node row, 7 positive pathway perturbation rows, 3 near-zero pathway rows, and 5 negative pathway rows. The strongest aggregate entries were component 523, target 3223, and the compound 523--target 3223 pathway.
...
Batch explicit-path candidates & 2 selected; 2 quantified & target 3223, 0.009835 & compound 523 to target 3223, 0.010074 & Transitional source; local checkpoint only \\
\bottomrule
\end{tabularx}
\end{table}

The aggregate ranking places the strongest positive pathway sensitivity on \texttt{compound:523;target:3223}, the strongest target sensitivity on \texttt{target:3223}, and the strongest component sensitivity on \texttt{compound:523}, but these ranks are computed from only the two perturbation-quantified cases and use the local predictor checkpoint. These results support the mechanism workflow shown in Figure~\ref{fig:mechanism-workflow} as triage rather than confirmed biological validation. In particular, negative score drops in the detailed report indicate that masking increased the model score and should not be interpreted as protective biological evidence.
...
The case evidence report contained 16 rows: 15 existing \msat{} Table 5-style candidates and one curated Zhishi--diarrhoea case. Two rows had mechanism/path support and were assigned Grade C, while 14 rows remained prediction-only Grade D candidates. Eight rows had automated literature search records, but no row had manually verified direct literature support. A targeted Direction 3 review queue derived from the batch interpretability report queued two parseable explicit-path candidates. The Zhishi--diarrhoea pair ranked first by positive pathway sensitivity and the Fragaria--altered-consciousness pair ranked second; both remained Grade C boundary cases after manual review. The ready-for-strong-evidence count was explicitly 0. Therefore, the current evidence workflow should be described as screening and prioritization rather than external validation equivalent to the original Table 5 or Table 6 analyses.
