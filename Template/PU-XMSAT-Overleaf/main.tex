...
We generated a conservative mechanism-interpretation report for selected cases (Table~\ref{tab:contribution}). In the Zhishi--diarrhoea case, the extracted key subgraph contained 11 nodes, 8 edges, and 14 mechanism paths. The largest node-level score drop was observed for target 3223 (drop 0.009835), and the largest path-level drop was observed for the compound 523 to target 3223 path (drop 0.010074). In the Fragaria--altered-consciousness case, the key subgraph contained two nodes and one edge, and the largest observed drop was 0.000021. Aggregating the quantified rows yielded 7 positive node perturbation rows and 10 positive path perturbation rows; the strongest aggregate entries remained target 3223 and the compound 523--target 3223 path.
